...
export const ForthDay: React.FC<TomorrowProps> = ({ forcastWeather,toggleSpanBlock }) => {
    const today = new Date()
    const tomorrowDate = new Date(today)
    tomorrowDate.setDate(today.getDate() + 3)
    ...
    const nextDayIndex = (todaysDate.getDay() + 3) % weekday.length
    
      const year = tomorrowDate.getFullYear()
      const month = String(tomorrowDate.getMonth() + 3).padStart(2, '0')
      ...
      toggleSpanBlock && (
              tomorrowsDay && tomorrowsDay.length > 0 && tomorrowToggle ? (
                <div className='p-5 overflow-x-auto'>
                  ...
                  bg-black cursor-pointer' onClick={toggleDayHandler}>{`${weekday[todaysDate.getDay() + 3]} ${year}-${month}-${day}`}</span>
